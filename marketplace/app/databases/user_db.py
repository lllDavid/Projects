...
def insert_user(user_details: UserDetails):
    ...
    cursor.execute("INSERT INTO users (username, email, password, role_id) VALUES (%s, %s, %s, %s)", 
                   (user_details.user.username, user_details.user.email, user_details.user.password, user_details.user.role.id))
...
def update_user(user_id: int, user_details: UserDetails):
    cursor = conn.cursor()
    cursor.execute("UPDATE users SET username = %s, email = %s, password = %s, role_id = %s WHERE user_id = %s", 
                   (user_details.user.username, user_details.user.email, user_details.user.password, user_details.user.role.id, user_id))
